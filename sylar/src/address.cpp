...
namespace sylar {
    ...
    bool Address::operator<(const Address& rhs) const {
        ...
        if(result == 0) return getAddrLen() < rhs.getAddrLen();
    ...
    IPv4Address::ptr IPv4Address::Create(const char* address, uint16_t port) {
        ...
        rt->m_addr.sin_port = toBigEndian(port);
    ...
    UnixAddress::UnixAddress() {
        ...
        m_length = offsetof(sockaddr_un, sun_path) + MAX_PATH_LEN;  // offsetof：用于计算结构体中的某个成员相比于结构体起始地址的字节偏移量
    }
